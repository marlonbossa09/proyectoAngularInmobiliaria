...
@Component({
  selector: 'app-modelo3d4',
  template: '<div id="modelo3D4"></div>',
})
export class Modelo3D4Component implements OnInit, OnDestroy {
  ...
  private init() {
    ...
    loader.load('assets/img/edificio_noche.glb', (gltf) => {
      this.modelo = gltf.scene;
      this.modelo.position.set(0, -7, 10);
...
this.scene.add(luzAmbiental);
    ...
    const contenedor = document.getElementById('modelo3D4');
  ...
  private async loadSkyboxHDR() {
    ...
    const texture = await loader.loadAsync('assets/img/fondogris.png');
