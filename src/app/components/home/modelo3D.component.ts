...
export class Modelo3DComponent implements OnInit, OnDestroy {
  ...
  private init() {
    ...
    loader.load('assets/img/logo.glb', (gltf) => {
    ...
    const luzAmbiental = new THREE.AmbientLight(0x800080, 1.0); // Color y intensidad
this.scene.add(luzAmbiental);
    // Agregar luz direccional
    const luzDireccional = new THREE.DirectionalLight(0x800080, 1);
